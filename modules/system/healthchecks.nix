...
in
{
  options.modules.healthchecks = {
    ...
    checkUrlFile = mkOption {
      ...
      description = "A file containing the URL to ping. It's recommended this is kept secret to avoid others pinging the URL.";
    };
  ...
  config = mkIf cfg.enable {
    systemd.services.healthchecks = {
      ...
      serviceConfig = {
        Type = "oneshot";
        ExecStart = "${pkgs.bash}/bin/bash -c '${pkgs.curl}/bin/curl -fsS -m 10 --retry 5 -o /dev/null $(${pkgs.coreutils}/bin/cat ${cfg.checkUrlFile})'";
      };
    };
