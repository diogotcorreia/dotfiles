{
  ...
  inputs = {
    ...
    nixpkgs-build-failure-notifier = {
      url = "github:diogotcorreia/nixpkgs-build-failure-notifier?ref=v0.3.1";
      inputs.nixpkgs.follows = "nixpkgs";
    };
    ...
    in
    {
      ...
      formatter = {
        ...
        x86_64-linux = pkgs.writeShellScriptBin "formatter" ''
          ...
          "${lib.getExe pkgs.deadnix}" --hidden --edit "''${DEADNIX_ARGS[@]}" -- "$@"
          "${lib.getExe pkgs.nixfmt-tree}" "''${NIXFMT_ARGS[@]}" -- "$@"
        '';
      };
    };
}
